...
func (b Builder) Build() (*Generation, error) {
	...
	tools, err := buildTools(b.Binding, b.Tools)
	if err != nil {
		return nil, err
	}
	g.tools = tools
...
// buildTools validates, orders, digests and names the tool set.
func buildTools(binding string, specs []protocol.ToolSpec) ([]Tool, error) {
	...
	if err := assignModelNames(binding, tools); err != nil {
		return nil, err
	}
	return tools, nil
}
