...
**Validate at every boundary.** All external input (HTTP, CLI args, env vars, files, MCP server responses) is untrusted until validated. Validate before it enters business logic, not inside it.
...
- Edge cases specific to the domain (e.g. malformed server responses, closed transports, cancelled contexts)
...
**Integration tests** — Write integration tests (tagged `//go:build integration`) for any code that crosses a process boundary: HTTP transports, subprocess (stdio) transports, real MCP servers. Integration tests live in `*_integration_test.go` files and are excluded from the default `go test ./...` run. Run them explicitly with `go test -tags integration -race ./...`.
